...
def Test3(test_val):
    for item in fruit:
        ...
        lst = GetResults()
        if item == 'banan':
            if test_val[0] in lst[-2] and '2' in lst[-2]:
            ...
            if test_val[0] in lst[-2] and '4' in lst[-2]:
                os.system('rlPass "Can remove' + str(item) + 'from shopping list"')
                # print('PASS - Can remove', item, 'from shopping list.')
            else:
                os.system('rlFail "Cant remove' + str(item) + 'from shopping list"')
...
def Test4(test_val):
    for item in fruit:
        ...
        CreateDataSet(test_val)
        lst = GetResults()
        if 'Tato položka není v seznamu' not in lst:
